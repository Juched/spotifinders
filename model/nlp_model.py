...
class SpotifinderModel(object):
    ...
    def __init__(self):
        ...
        # get feature model and standardize
        X = feature_matrix
        self.scaler.fit(X)
        ...
        # diff = self.dance_reg.predict(X_test) - y_test
        # print(diff)
        ...
        # diff = self.energy_reg.predict(X_test) - y_test
        # print(diff)
        # print(diff.sum(axis=0)**2)
        ...
        # diff = self.valence_reg.predict(X_test) - y_test
        # print(diff)
        # print(diff.sum(axis=0)**2)
    ...
    def get_vector(self, sentence: str) -> np.array:
        ...
        fv_st = self.scaler.fit(feature_vector)

        return self.dance_reg.predict(fv_st), self.energy_reg.predict(fv_st), self.valence_reg.predict(fv_st)
